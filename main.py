        ...
        if self.auto:
            for i in range(self.SENSOR_INTERVAL[0], self.SENSOR_INTERVAL[1]):
                ...
                for j in range(self.DAY_INTERVAL[0], self.DAY_INTERVAL[1]):
                    DAY = j
                    fi, ri, si, min_dist = Run_all.run(Run_all(), SENSOR, DAY, self.use_avg, self.draw_chart, n_peaks_co2_to_find, n_peaks_tvoc_to_find, n_peaks_pm25_to_find, n_peaks_temp_to_find, n_peaks_humidity_to_find, self.max_num_intervals, max_disp_small, max_disp_large, n_peaks_for_large_tollerance)  # (flag: use_avg, draw_chart)
            ...
            with open('statistiche.txt', mode='a+') as stat_file:
                ...
                stat_writer.writerow(["Avg min dist: {}".format(min_dist / count_test)])
...
for i in range(2, 10):
    for j in range(i+1,25):
        Main.run(Main(), 10, 7, 2, 3, 2, i, j, 3)